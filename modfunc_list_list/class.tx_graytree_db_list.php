 ...
 * The original file could not be used only because of the object intantiation at the bottom of the file.
 *
...
require_once(PATH_txgraytree.'modfunc_list_list/class.tx_graytree_db_list_extra.inc');
...
class tx_graytree_db_list {
	...
	var $valueArray;			// field and value pairs to build a where clause from the table
	...
	var $control;				//Array of tree leaf names and leaf data classes 
		...
		$this->control = array (
			'category' => array ('selType' => 'txcommerceCategory', 
								'selClass' => 'tx_commerce_stdselectionCategoryData',
								'parent'   => 'parent_category'),
			'product' => array ( 'selType' => 'txcommerceProduct', 
								'selClass' => 'tx_commerce_stdselectionProductData',
								'parent'   => 'categories')
								); */
	...
	function init()	{
		...
		if (!$this->scriptNewWizard)	{
			$this->scriptNewWizard = $BACK_PATH.'db_new.php';
		}
	...
	function main()	{
		...
		$formProduced = false;
		$linkparam = array();	
		$parent_uid = $this->controlArray['uid'];
		...
		$dblist = t3lib_div::makeInstance('graytreeRecordList');
		...
		foreach ($this->control as $type => $controldat) {
			...
			$this->resCountAll = $treedb->countSubRecords($parent_uid);
			if ($treedb->table)	{
				$linkparam[] = '&edit['.$treedb->table.'][-'.$parent_uid.']=new';
				$tmpDefVals = '&defVals['.$treedb->table.']['.$controldat['parent'].']=' .$parent_uid;
				$defVals .= $tmpDefVals;
			} 
			
				// If there is access to the page, then render the list contents and set up the document template object:
			if ($access && $this->resCountAll)	{
				$this->table = ($treedb->table ? $treedb->table : $this->table);
				$dblist->allFields = ($this->MOD_SETTINGS['bigControlPanel'] || $this->table) ? 1 : 0;
				...
				$uid = $treedb->getSubRecordsIdList($parent_uid);
...
// Include extension?
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/graytree/modfunc_list_list/class.tx_graytree_db_list.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/graytree/modfunc_list_list/class.tx_graytree_db_list.php']);
}
